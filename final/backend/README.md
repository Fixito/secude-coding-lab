...
API REST Express/TypeScript servant de support pédagogique pour illustrer des vulnérabilités web courantes (OWASP Top 10) et leurs corrections.
...
- **Corrigé :** comparaison `req.user.userId === id` après vérification du JWT
...
- **Corrigé :** Double Submit Cookie Pattern — token généré serveur, transmis dans cookie et formulaire, comparé à chaque requête
